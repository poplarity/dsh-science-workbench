// Client half of dsh-bio-workbench as a static Cordis plugin (Phase 2).
//
// STATIC translation of the validated dynamic prototype `client/index.js`.
// Differences from the dynamic version:
//
//   - `host.call(method, args)` -> `ctx.remote.bioWorkbench.<method>(args)`
//     (the Host half exposes `@Remote` methods on the BioWorkbenchService).
//   - `styles.insert(css)` / `React` builtins -> a plain `import React` plus a
//     one-shot `<style>` injection (browser-safe, no build CSS pipeline).
//   - `slots.inject`/`slots.register` are unchanged (same Client slots API).
//
// The client bundle is produced from this file by the harness web pipeline
// (`exports["./client"]` + `dsh.client`); `ctx.remote.bioWorkbench` is wired
// from the typert-generated remote-client descriptor of lib/index.ts.

import React from "react"

export const name = "dsh-bio-workbench"
export const inject = ["slots", "remote", "remote.bioWorkbench"]
...
export function apply(ctx: any) {
  ...
  // One-shot CSS injection (browser-safe; guarded against HMR re-entry).
  ...
  const remote = ctx.remote.bioWorkbench
  const h: any = React.createElement

  function useProject() {
    const [data, setData] = React.useState<any>(null)
    const [selectedName, setSelectedName] = React.useState<any>(null)
    const [error, setError] = React.useState<any>(null)
    const [loading, setLoading] = React.useState(false)

    function fetch(name: any) {
      setLoading(true)
      remote.getProject(name ? { name: name } : {})
        .then(function (d: any) {
          setData(d)
          setError(null)
          if (!name) {
    ...
    function call(method: string, args: any) {
      ...
      return remote[method](a)
    }

    function select(name: any) {
      setSelectedName(name)
      fetch(name)
    }

    function createProject(newName: string) {
      return remote.initProject({ name: newName }).then(function () {
    ...
    function setDir(dir: string) {
      return remote.setProjectsDir({ dir: dir }).then(function () {
    ...
    function reveal(path: string, isDir: boolean) {
      return call("revealInFinder", { path: path, isDir: isDir }).catch(function (e: any) { console.error(e) })
    }
  ...
  function fmtTime(iso: string) {
    if (!iso) return ""
    try { return String(iso).replace("T", " ").slice(0, 19) } catch { return String(iso) }
  }

  function figureOf(figures: any, path: string) {
    if (!figures) return null
    for (let i = 0; i < figures.length; i++) if (figures[i].path === path) return figures[i]
    return null
  }

  function Lightbox(props: any) {
    if (!props.item) return null
    const it = props.item
    return h("div", { className: "biowb-lb", onClick: props.onClose },
      it.mime === "application/pdf"
        ? h("iframe", { className: "biowb-lb-pdf", src: it.src, title: "preview", onClick: function (e: any) { e.stopPropagation() } })
        : h("img", { className: "biowb-lb-img", src: it.src, alt: "preview", onClick: function (e: any) { e.stopPropagation() } }),
      h("button", { className: "biowb-lb-close", onClick: props.onClose }, "✕")
    )
  }

  function FigureImage(props: any) {
  ...
  function FeedbackBox(props: any) {
    ...
    function submit() {
      ...
      }).catch(function (e: any) { setMsg("失败: " + (e && e.message ? e.message : e)) })
    }

    return h("div", { style: { marginTop: 10 } },
      (props.feedback && props.feedback.length) ? h("div", { className: "biowb-fb-list" },
        (props.feedback || []).map(function (f: any, i: number) {
          return h("div", { key: i, className: "biowb-muted" }, "💬 ", f.text, " · ", fmtTime(f.at))
        })
      ) : null,
      h("div", { className: "biowb-row", style: { marginTop: 4 } },
        h("input", { className: "biowb-input", type: "text", value: text, placeholder: "反馈：改 x 轴 / 换配色 / 加 p 值…", onChange: function (e: any) { setText(e.target.value) } }),
  ...
  function ArtifactDetail(props: any) {
    ...
    function toggleCode() {
      if (panel !== "code") {
        if (code === null) {
          props.call("getCellCode", { cellId: art.producedBy }).then(function (r: any) {
  ...
  function DeleteCellButton(props: any) {
  ...
  function ProjectHeader(props: any) {
    ...
    function create() {
      ...
      if (!n) return
      props.createProject(n).then(function () { setShowNew(false); setNewName("") }).catch(function (e: any) { console.error(e) })
    }

    function applyDir() {
      const d = dir.trim()
      if (!d) return
      props.setDir(d).then(function () { setShowDir(false); setDir("") }).catch(function (e: any) { console.error(e) })
    }

    return h("div", null,
      h("div", { className: "biowb-head" },
        h("span", { className: "biowb-title" }, "项目"),
        h("select", { className: "biowb-select", value: props.selectedName || "", onChange: function (e: any) { props.select(e.target.value) } },
          (props.projects || []).map(function (p: any) { return h("option", { key: p.name, value: p.name }, p.name) })
        ),
      ...
      showNew ? h("div", { className: "biowb-row", style: { marginBottom: 12 } },
        h("input", { className: "biowb-input", type: "text", value: newName, placeholder: "项目名（如 chip_tss）", style: { maxWidth: 240 }, onChange: function (e: any) { setNewName(e.target.value) } }),
        h("button", { className: "biowb-btn primary", onClick: create }, "创建")
      ) : null,
      showDir ? h("div", { className: "biowb-row", style: { marginBottom: 12 } },
        h("input", { className: "biowb-input", type: "text", value: dir, placeholder: "绝对路径，如 /Users/wangyian/Documents/DSH/bio-projects", style: { maxWidth: 420 }, onChange: function (e: any) { setDir(e.target.value) } }),
        h("button", { className: "biowb-btn primary", onClick: applyDir }, "设置目录")
      ) : null
    )
  }

  function BioWorkbench(props: any) {
    const p = useProject()
    const cur = p.data && p.data.current
    const cells = cur ? (cur.manifest.cells || []) : []
    const figures = cur ? (cur.figures || []) : []
    const [selectedCellId, setSelectedCellId] = React.useState<any>(null)
    const [lightbox, setLightbox] = React.useState<any>(null)
    const fmLabel = (p.data && p.data.platform === "win32") ? "资源管理器" : "Finder"

    const effectiveCellId = selectedCellId && cells.some(function (c: any) { return c.id === selectedCellId })
      ? selectedCellId
      : (cells.length ? cells[cells.length - 1].id : null)

    const selectedCell = cells.find(function (c: any) { return c.id === effectiveCellId })
          ...
          h("div", { className: "biowb-col-right biowb-section" },
            ...
            selectedCell ? h("div", null,
              ...
              (selectedCell.artifacts && selectedCell.artifacts.length)
                ? selectedCell.artifacts.map(function (ap: any) {
                    const art = (cur.manifest.artifacts || []).find(function (a: any) { return a.path === ap })
  ...
  function BioRunCellCard(props: any) {
    const p = useProject()
    const cur = p.data && p.data.current
  ...
  slots.inject("conversation.view", function () {
    slots.register(
      ...
      function (props: any) { return h(BioWorkbench, { inputActions: props && props.inputActions }) }
    )
  })
  ...
  slots.inject("tool.call.toolview", function () {
    slots.register(
      ...
      function (props: any) { return h(BioRunCellCard, { inputActions: props && props.inputActions }) }
    )
  })
}
