...
>
> **Integration in this workbench:** compose publication-grade multi-panel
> figures as `bio_run_cell` output. The helper `kernel.py` ships beside this
> SKILL.md — copy it into the project's `code/` once:
> `cp <this-skill-dir>/kernel.py <project>/code/figure-composer-kernel.py`, then
> in each cell `exec(open("code/figure-composer-kernel.py").read())`. Run the
> workflow below one panel at a time; `figure-style` (also bundled) is loaded
> alongside.
