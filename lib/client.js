...
window.__ModuleLoader__.load({
  ...
  factory: function (require) {
    ...
    var name = "dsh-science-workbench";
    ...
    function apply(ctx) {
      ...
      function useProject() {
        ...
        // Open the Host's native directory picker and set the projects root.
        function pickDir() {
          if (workspaces && typeof workspaces.pickDirectory === "function") {
            return workspaces.pickDirectory().then(function (path) {
              if (path) return setDir(path);
              return null;
            });
          }
          return null;
        ...
        return { data: data, error: error, loading: loading, selectedName: selectedName, projects: projects, select: select, createProject: createProject, setDir: setDir, pickDir: pickDir, reveal: reveal, refresh: refresh, call: call };
      }
      ...
      function StepItem(props) {
        ...
        return h("div", { className: "biowb-step" + (props.active ? " active" : ""), onClick: function () { props.onSelect(c.id); } },
          h("div", { className: "biowb-row" },
            ...
            c.final === true ? h("span", { className: "biowb-tag kind" }, "成品") : null
          ),
      ...
      function ProjectHeader(props) {
        ...
        return h("div", null,
          h("div", { className: "biowb-head" },
            ...
            h("span", { className: "biowb-spacer" }),
            h("button", { className: "biowb-iconbtn", title: "设置存储目录", onClick: function () {
              if (props.pickDir) { props.pickDir(); } else { setShowDir(!showDir); setShowNew(false); }
            } }, "⚙"),
      ...
      function BioWorkbench(props) {
        ...
        var visibleCells = q ? cells.filter(function (c) {
          return (c.id || "").toLowerCase().indexOf(q) !== -1
            || (c.title || "").toLowerCase().indexOf(q) !== -1
            || (c.status || "").toLowerCase().indexOf(q) !== -1;
        }) : cells;

        var effectiveCellId = selectedCellId && visibleCells.some(function (c) { return c.id === selectedCellId; })
          ? selectedCellId
          : (visibleCells.length ? visibleCells[visibleCells.length - 1].id : null);
        ...
        return h("div", { className: "biowb" },
          ...
          h(ProjectHeader, {
            ...
            createProject: p.createProject, setDir: p.setDir, pickDir: p.pickDir, reveal: p.reveal,
            refresh: p.refresh, loading: p.loading, fmLabel: fmLabel
          }),

          p.projects.length === 0 ? h("div", { className: "biowb-section" },
            h("h3", null, "第一步：设置工作目录"),
            h("div", { className: "biowb-muted", style: { marginBottom: 10 } }, "还没有分析项目。先指定项目存放目录（默认 ~/bio-projects），再创建或让 agent 用 bio_init_project 建项目。"),
            h("button", { className: "biowb-btn primary", onClick: function () { if (p.pickDir) p.pickDir(); } }, "选择目录")
            ...
            h("div", { className: "biowb-cols" },
              h("div", { className: "biowb-col-left biowb-section" },
                h("h3", null, "分析步骤"),
                h("input", { className: "biowb-input", type: "text", value: search, placeholder: "检索 cell（id / 标题 / 状态）…", style: { marginBottom: 10 }, onChange: function (e) { setSearch(e.target.value); } }),
                visibleCells.length === 0 ? h("div", { className: "biowb-muted" }, cells.length ? "无匹配结果。" : "暂无步骤。") : null,
                visibleCells.map(function (c) {
              ...
              h("div", { className: "biowb-col-right biowb-section" },
                ...
                selectedCell ? h("div", null,
                  h("div", { className: "biowb-row", style: { marginBottom: 6 } },
                    ...
                    selectedCell.final === true ? h("span", { className: "biowb-tag kind" }, "成品") : null,
                    h("span", { className: "biowb-spacer" }),
                    h("button", { className: "biowb-btn", onClick: function () { p.call("markCellFinal", { cellId: selectedCell.id, final: !(selectedCell.final === true) }).then(function () { p.refresh(); }); } }, selectedCell.final === true ? "取消成品" : "标记成品"),
      ...
      slots.inject("conversation.view", function () {
        slots.register(
          { name: "conversation.view", id: "dsh-science-workbench", order: 20, label: "分析工作台" },
