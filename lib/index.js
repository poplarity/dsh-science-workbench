// Host half of dsh-science as a STATIC Cordis plugin (plain ESM).
//
...
export function apply(ctx) {
  ...
  const workspaceRoot =
    ...
    process.cwd()
  const settingsFile = workspaceRoot + "/.dsh-science.json"
  ...
  async function gitInit(root) {
    const cmd = IS_WIN
      ? 'git init -q 2>$null; git config user.email "dsh-science@local" 2>$null; git config user.name "dsh-science" 2>$null'
      : 'git init -q 2>/dev/null; git config user.email "dsh-science@local" 2>/dev/null; git config user.name "dsh-science" 2>/dev/null'
    await runShell(cmd, root, 30000)
  }
  ...
  async function snapshotEnvironment(root, language) {
    ...
    if (language === "python") {
      ...
      const lock = "# dsh-science environment lock\n# interpreter: " + interpreter + "\n# generated: " + new Date().toISOString() + "\n\n" + String(pkgs.stdout.text || "")
      ...
      await writeText(root + "/environment.lock", "# dsh-science environment lock\n# interpreter: " + interpreter + "\n")
